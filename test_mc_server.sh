...
#Clear screen so its easy to read the prompts
clear
...
cat << EOF > start.sh
#!/bin/bash
java -jar -Xmx4G -Xms4G myserver.jar nogui
EOF
...
wget -cO -  $server_download > myserver.jar;;
...
echo "Your server files are made! If you need help using them read the usage section on the Github page you got this from"
sleep 2
echo "Your server is in $dir_name, to get to it type 'cd /home/$whoami/$dir_name'"
echo "###################################################################################################################"
